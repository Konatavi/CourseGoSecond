...
//For UPDATE request
func (ar *ArticleRepository) UpdateById(id int, newArticle *models.Article) (*models.Article, error) {
	_, ok, err := ar.FindArticleById(id)
	if err != nil {
		return nil, err
	}
	if ok {
		query := fmt.Sprintf("update %s set title = $1, author = $2, content = $3 where id=$4", tableArticle)
		_, err = ar.store.db.Exec(query, newArticle.Title, newArticle.Author, newArticle.Content, id)
		if err != nil {
			return nil, err
		}
	}
